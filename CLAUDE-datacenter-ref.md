...
- **Cost model**: `gemini-3.5-flash` (paid) runs the entire office
...
- **Stop logic**: on error, auto-fix first (try a minimal fix, then a
  ...
  never halt the run.
- **Models**: `gemini-3.5-flash` for the office simulation
  (`gemini-3.5-flash` is deprecated — do not reintroduce it),
